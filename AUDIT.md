...
## 2026-09-09 — URGENT: unverified claims removed sitewide

**Trigger:** the owner reported that clients (ZYEN, Owain) had challenged the ISO 9001
claim, and that FABRIOZA does not hold it. Also that free design mockups are not offered.

**1. ISO 9001 — removed everywhere it was a FABRIOZA claim.** 304 mentions across 86 files:
page copy, titles, meta descriptions, OG/Twitter tags, JSON-LD (including the Organization
`hasCredential` credential node), llms.txt, and 14 separate strings in the SPA bundle (hero
badge, trust badges, stats strip, factory-tour copy and chips, collections heading, why-us
card, QC badges, FAQ answer, footer blurb, footer badge list, runtime schema description and
runtime FAQ answer).

**Deliberately KEPT: 11 educational mentions** in blog articles that advise readers what to
look for in *any* factory ("Look for ISO 9001 (quality management), BSCI...", "How do I know
if an ISO 9001 certificate is real?", the three-check rule, the certifications guide's own
subject matter). These are advice, not claims; removing them would gut the site's best
explainer article. Verified: zero remaining sentences claim FABRIOZA holds ISO 9001.

Replacements use only what is defensible: "amfori BSCI audited" and "documented 4-stage QC
with photo records", plus plain descriptors ("Sialkot factory", "export-grade").

**2. Free mockups — removed.** 174 instances across the site and bundle, including the
variants the first pass missed ("free kit mockup", "Get a Free Jersey Mockup", "Free visual
mockup", "Free digital mockup"). Mockups are still described as a service, just not as free.
The homepage process step is now "Get Your Mockup". Only "free quote with a mockup" remains,
which is accurate — the quote is free.

**3. MOQ messaging simplified** per instruction: homepage title is now
"Custom Clothing Manufacturer in Sialkot | MOQ from 20 Pieces | FABRIOZA" and the three
meta/OG descriptions say "MOQ from 20 pieces." instead of the confusing
"Trial orders from 20 pieces, standard MOQ 50."

**Incident during this work (and the rule it produced).** The first scrub script included
generic tidy-up rules, one of which was `'..' -> '.'`. Applied to `dist/assets/*.js` it
mangled `...` spread operators and **blanked the homepage**; a second rule (`'  ' -> ' '`)
churned 18,000 lines of whitespace across 89 files. Caught by the browser check, not by
`node --check` (which fails on this bundle regardless, since it parses an ES module as
CommonJS). The whole working tree was reset with `git checkout -- .` and redone: bulk rules
now exclude `.js` entirely and the destructive cleanups are gone; the bundle receives only
exact, hand-verified string swaps followed by a browser load. Ellipses in 404.html,
tech-pack-clothing-guide and sample-order verified intact afterwards.

Cache-bust bumped to `?v=20260908`. schema_verify 92/92, homepage renders 21 sections with
zero JS errors, all key pages 200.

**Still open, needs the owner's answer:**
- **amfori BSCI** — if ISO 9001 was not documented, is the BSCI audit? It is now the only
